...
export class FIGCharacterPaddingInfo {
    ...
    constructor(character: number[][]) {
        ...
        this._topDownCharacters = Array(width).fill(ASCIICodes.SPACE);
        this._bottomUpCharacters = Array(width).fill(ASCIICodes.SPACE);
        this._leftToRightCharacters = Array(height).fill(ASCIICodes.SPACE);
        this._rightToLeftCharacters = Array(height).fill(ASCIICodes.SPACE);
        ...
        for (let column = 0; column < width; column++) {
            for (let row = 0; row < height; row++) {
                ...
                // Top down
                if (asciiCharacter !== ASCIICodes.SPACE && this._topDownDistances[column] == Infinity) {
                    this._topDownDistances[column] = row;
                    this._topDownCharacters[column] = asciiCharacter;
                }

                // Bottom up
                if (asciiCharacter !== ASCIICodes.SPACE) {
                    this._bottomUpDistances[column] = height - row - 1;
                    this._bottomUpCharacters[column] = asciiCharacter;
                }

                // Left-to-right
                if (asciiCharacter !== ASCIICodes.SPACE && this._leftToRightDistances[row] == Infinity) {
